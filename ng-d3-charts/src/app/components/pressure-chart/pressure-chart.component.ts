import { ChangeDetectionStrategy, Component, ElementRef, Input, OnChanges, ViewChild } from '@angular/core';
import * as d3 from 'd3';

import { PressureData } from '../../pressure-data';

@Component({
  selector: 'app-pressure-chart',
  templateUrl: './pressure-chart.component.html',
  styleUrls: ['./pressure-chart.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PressureChartComponent implements OnChanges {

  @ViewChild('chart', { static: false })
  chartElement: ElementRef;

  parseDate = d3.timeParse('%d-%m-%Y');

  @Input()
  pressure: PressureData[];

  private svgElement: HTMLElement;
  private chartProps: any;

  constructor() { }

  ngOnChanges() {
    if (this.pressure && this.chartProps) {
      this.updateChart();
    } else if (this.pressure) {
      this.buildChart();
    }
  }

  formatDate() {
    this.pressure.forEach(ms => {
      if (typeof ms.date === 'string') {
        ms.date = this.parseDate(ms.date);
      }
    });
  }
  updateChart() {
    const tempThis = this;
    this.formatDate();

    // Scale the range of the data again
    this.chartProps.x.domain(d3.extent(this.pressure, (d) => {
      if (d.date instanceof Date) {
        return d.date.getTime();
      }
    }));

    this.chartProps.y.domain([0, d3.max(this.pressure, (d) => Math.max(d.pressure2, d.pressure1))]);

    // Select the section we want to apply our changes to
    this.chartProps.svg.transition();

    // Make the changes to the line chart
    this.chartProps.svg.select('.line.line1') // update the line
      .attr('d', this.chartProps.valueline(this.pressure));

    this.chartProps.svg.select('.line.line2') // update the line
      .attr('d', this.chartProps.valueline2(this.pressure));

    this.chartProps.svg.select('.x.axis') // update x axis
      .call(this.chartProps.xAxis);

    this.chartProps.svg.select('.y.axis') // update y axis
      .call(this.chartProps.yAxis);
  }
  buildChart() {
    this.chartProps = {};
    this.formatDate();

    // Set the dimensions of the canvas / graph
    const margin = {
      top: 30,
      right: 20,
      bottom: 30,
      left: 50
    };

    const width = 600 - margin.left - margin.right;
    const height = 270 - margin.top - margin.bottom;

    // Set the ranges
    this.chartProps.x = d3.scaleTime().range([0, width]);
    this.chartProps.y = d3.scaleLinear().range([height, 0]);

    // Define the axes
    const xAxis = d3.axisBottom(this.chartProps.x);
    const yAxis = d3.axisLeft(this.chartProps.y).ticks(5);

    const tempThis = this;


    // Define the line
    const valueline = d3.line<PressureData>()
      .x((d) => {
        if (d.date instanceof Date) {
          return tempThis.chartProps.x(d.date.getTime());
        }
      })
      .y((d) => {
        console.log('Close pump');
        return tempThis.chartProps.y(d.pressure2);
      });

    // Define the line
    const valueline2 = d3.line<PressureData>()
      .x((d) => {
        if (d.date instanceof Date) {
          return tempThis.chartProps.x(d.date.getTime());
        }
      })
      .y((d) => {
        console.log('Open pump');
        return tempThis.chartProps.y(d.pressure1);
      });

    const svg = d3.select(this.chartElement.nativeElement)
      .append('svg')
      .attr('width', width + margin.left + margin.right)
      .attr('height', height + margin.top + margin.bottom)
      .append('g')
      .attr('transform', `translate(${margin.left},${margin.top})`);

    // Scale the range of the data
    this.chartProps.x.domain(
      d3.extent(tempThis.pressure, (d) => {
        if (d.date instanceof Date) {
          return (d.date as Date).getTime();
        }
      }));
    this.chartProps.y.domain([0, d3.max(this.pressure, (d) => {
      return Math.max(d.pressure2, d.pressure1);
    })]);

    // Add the valueline2 path.
    svg.append('path')
      .attr('class', 'line line2')
      .style('stroke', 'green')
      .style('fill', 'none')
      .attr('d', valueline2(tempThis.pressure));

    // Add the valueline path.
    svg.append('path')
      .attr('class', 'line line1')
      .style('stroke', 'black')
      .style('fill', 'none')
      .attr('d', valueline(tempThis.pressure));


    // Add the X Axis
    svg.append('g')
      .attr('class', 'x axis')
      .attr('transform', `translate(0,${height})`)
      .call(xAxis);

    // Add the Y Axis
    svg.append('g')
      .attr('class', 'y axis')
      .call(yAxis);

    // Setting the required objects in chartProps so they could be used to update the chart
    this.chartProps.svg = svg;
    this.chartProps.valueline = valueline;
    this.chartProps.valueline2 = valueline2;
    this.chartProps.xAxis = xAxis;
    this.chartProps.yAxis = yAxis;
  }
}
